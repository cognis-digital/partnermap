# Comprehensive installer for cognis-digital/partnermap (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# partnermap is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "partnermap"
$Url  = "git+https://github.com/cognis-digital/partnermap.git"
$Git  = "https://github.com/cognis-digital/partnermap.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: partnermap"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: partnermap"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: partnermap"; exit 0 }
}
