...
How **partnermap** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `partnermap` lives in the **Maritime domain awareness** cluster.
...
| from | relation | to |
|---|---|---|
| `partnermap` | JSON findings compose with | [`maritimeint`](https://github.com/cognis-digital/maritimeint), [`uaslog`](https://github.com/cognis-digital/uaslog), [`corpmap`](https://github.com/cognis-digital/corpmap) |
| `partnermap` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `partnermap` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `partnermap` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the maritime domain awareness cluster.** `partnermap` output feeds the next tool:
```bash
partnermap ... --format json > out.json          # this tool's findings (see `partnermap --help`)
maritimeint ... < out.json                      # the cluster sibling consumes them
```
...
partnermap ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
partnermap ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
partnermap ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `partnermap`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `partnermap` slots
into the **Sanctions-evasion screening**, **Counter-UAS maritime** stacks below.
