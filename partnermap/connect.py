"""Native cognis-connect emit for partnermap — forward findings to any platform.

Maps partnermap's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    partnermap ... --format json | partnermap-emit --to stix
    partnermap-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out['title'] = rec.get('partner', '') + ' - ' + rec.get('account', '')
        out['severity'] = 'medium'
        out['type'] = 'renewal'
        out['description'] = f"Renewal alert for account {rec.get('account', '')}"
        out['tags'] = ['renewal']
