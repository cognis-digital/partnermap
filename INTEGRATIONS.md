...
**partnermap** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `partnermap` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
partnermap ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
partnermap ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
partnermap ... --format json | cognis-connect emit --to brief   # analyst brief via your /v1 fleet
```
...
works whatever `partnermap` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="partnermap") for rec in records]   # records = your JSON output
